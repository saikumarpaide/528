...
import { catalogValidationWatcherPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [catalogValidationWatcherPlugin],
    });

    await request(server).get('/api/catalog-validation-watcher/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/catalog-validation-watcher/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/catalog-validation-watcher/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/catalog-validation-watcher/todos/${createdTodoItem.id}`)
  ...
  it('should create TODO item with catalog information', async () => {
    const { server } = await startTestBackend({
      features: [
        catalogValidationWatcherPlugin,

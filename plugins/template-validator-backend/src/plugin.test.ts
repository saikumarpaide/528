...
import { templateValidatorPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [templateValidatorPlugin],
    });

    await request(server).get('/api/template-validator/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/template-validator/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/template-validator/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/template-validator/todos/${createdTodoItem.id}`)
